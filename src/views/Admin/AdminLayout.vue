...
.admin-content.admin-expanded {
  margin-left: 12px; /* more gap when expanded */
}
.admin-content.admin-collapsed {
  margin-left: 4px; /* smaller gap when collapsed */
}

.collapse-stripe {
  position: absolute;
  right: 0px; /* moved 4px further outside the drawer */
  top: 0;
  bottom: 0;
  width: 22px;
...
.collapse-stripe .v-icon { /* base transform/transition */
...
.collapse-stripe.is-collapsed {
  right: 0px; /* push further to the right */
  width: 14px; /* visually smaller clickable area */
  background: transparent;
  box-shadow: none;
}
.collapse-stripe.is-collapsed:hover {
  background: rgba(0,0,0,0.02); /* minimal hover effect */
}
</style>
