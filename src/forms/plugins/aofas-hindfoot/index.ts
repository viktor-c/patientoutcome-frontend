/**
 * AOFAS Hindfoot Form Plugin
 * American Orthopaedic Foot & Ankle Society Ankle-Hindfoot Score
 */

import AofasHindfootForm from './AofasHindfootForm.vue'
import { translations } from './translations'
import { calculateScore, validateFormData, getInitialData, generateMockData } from './scoring'
import type { FormPlugin } from '../../types'

const plugin: FormPlugin = {
  metadata: {
    id: '67b4e612d0feb4ad99ae2e88', // Must match backend FormTemplate _id
    name: 'AOFAS Hindfoot',
    description: 'American Orthopaedic Foot & Ankle Society Ankle-Hindfoot Score - Clinical rating system for hindfoot and ankle assessment',
    version: '1.0.0',
    supportedLocales: ['en', 'de']
  },
  
  component: AofasHindfootForm,
  translations,
  calculateScore,
  validateFormData,
  getInitialData,
  generateMockData,
  
  schema: {
    type: 'object',
    properties: {
      hindfoot: {
        type: 'object',
        properties: {
          q1: { type: ['number', 'null'], enum: [null, 40, 30, 20, 0] },
          q2: { type: ['number', 'null'], enum: [null, 10, 7, 4, 0] },
          q3: { type: ['number', 'null'], enum: [null, 10, 7, 4, 0] },
          q4: { type: ['number', 'null'], enum: [null, 5, 4, 2, 0] },
          q5: { type: ['number', 'null'], enum: [null, 5, 3, 0] },
          q6: { type: ['number', 'null'], enum: [null, 8, 4, 0] },
          q7: { type: ['number', 'null'], enum: [null, 8, 4, 0] },
          q8: { type: ['number', 'null'], enum: [null, 6, 3, 0] },
          q9: { type: ['number', 'null'], enum: [null, 8, 0] },
          q10: { type: ['number', 'null'], enum: [null, 10, 5, 0] }
        }
      }
    }
  }
}

export default plugin
export { AofasHindfootForm, translations, calculateScore, validateFormData, getInitialData, generateMockData }
