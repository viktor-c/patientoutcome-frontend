...
const originalValues = ref<Record<string, number | null>>({})
...
watch(() => props.modelValue, () => {
  if (!hasInitializedOriginalValues.value) {
    questionsData.value.forEach((question) => {
      originalValues.value[question.key] = getCurrentValue(question.key)
    })
...
function getQuestionColor(questionKey: string, index: number): string {
  ...
  if (getCurrentValue(questionKey) !== null) return 'success'
  return 'grey'
}
...
function goToQuestion(index: number) {
  ...
  // Allow navigation to any question (removed restriction)
...
const navigationItems = computed<QuestionNavItem[]>(() => {
  return questionsData.value.map((question, index) => ({
    index,
